  ...
  return (
    <div className="space-y-4">
      <div className="flex items-end justify-between gap-3">
        <div><h1 className="font-display text-2xl font-semibold text-on-surface">告警中心</h1><p className="mt-1 text-sm text-muted">查看当前告警实例与不可变状态变化记录。</p></div>
        <div className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm font-semibold text-red-700">当前列表 {activeCount} 个活跃告警</div>
      </div>
      ...
      <DataPanel title="通知策略" meta={`${policiesQuery.data?.length ?? 0} policies`}>
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <div className="rounded-md border border-outline bg-surface-low/60 p-4">
            ...
            <label className="mt-3 block text-xs font-semibold text-on-surface">策略名称<input className="mt-1.5 h-9 w-full rounded-md border border-outline bg-white px-3 text-sm outline-none focus:border-primary" value={policyName} onChange={(event) => setPolicyName(event.target.value)} placeholder="例如：支付团队值班" /></label>
            <label className="mt-3 block text-xs font-semibold text-on-surface">适用范围<select className="mt-1.5 h-9 w-full rounded-md border border-outline bg-white px-3 text-sm outline-none focus:border-primary" value={policyServiceId} onChange={(event) => setPolicyServiceId(event.target.value)}><option value="">全局策略</option>{(servicesQuery.data ?? []).map((service) => <option key={service.id} value={service.id}>{service.displayName || service.name}</option>)}</select></label>
            <label className="mt-3 block text-xs font-semibold text-on-surface">Receiver 标识<input className="mt-1.5 h-9 w-full rounded-md border border-outline bg-white px-3 font-mono text-sm outline-none focus:border-primary" value={receiverName} onChange={(event) => setReceiverName(event.target.value)} placeholder="pay-oncall" /></label>
            {createPolicyMutation.error ? <ErrorLine message={(createPolicyMutation.error as Error).message} /> : null}
            <button className="mt-3 inline-flex h-9 w-full items-center justify-center rounded-md bg-primary text-sm font-semibold text-white disabled:opacity-45" disabled={!policyName.trim() || !receiverName.trim() || createPolicyMutation.isPending} onClick={() => createPolicyMutation.mutate()}>创建通知策略</button>
          </div>
        </div>
      </DataPanel>
...
function InstanceState({ state }: { state: 'pending' | 'firing' | 'resolved' }) {
  ...
  return <span className={`inline-flex items-center gap-1 rounded border px-2 py-0.5 text-[11px] font-semibold ${style}`}><Icon className="h-3 w-3" />{state}</span>;
}

function formatTime(value: string) { return value ? new Date(value).toLocaleString('zh-CN', { hour12: false }) : '-'; }
function ErrorLine({ message }: { message: string }) { return <div className="mb-3 rounded border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{message}</div>; }
