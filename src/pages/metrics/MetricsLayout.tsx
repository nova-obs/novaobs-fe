import { Outlet } from 'react-router-dom';
import { Bell, Gauge, LayoutDashboard, PlugZap } from 'lucide-react';
import { ModuleWorkbench } from '../../components/navigation/ModuleWorkbench';
import type { ModuleRailItem } from '../../components/navigation/ModuleRail';

const items: ModuleRailItem[] = [
  { to: '/metrics/overview', label: '监控总览', description: '产品指标健康与基础设施关键信号', icon: Gauge },
  { to: '/metrics/dashboard', label: 'Dashboard', description: '打开 Grafana 工作区', icon: LayoutDashboard, end: true },
  { to: '/metrics/alerts', label: '指标告警', description: '产品级 PromQL 与 MetricsQL 告警规则', icon: Bell },
  { to: '/metrics/integrations', label: '指标接入', description: '产品指标来源、写入目标与生命周期', icon: PlugZap },
];

export function MetricsLayout() {
  return <ModuleWorkbench module="metrics" title="监控" ariaLabel="Metrics 模块导航" items={items}><Outlet /></ModuleWorkbench>;
}
