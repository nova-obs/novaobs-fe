...
import type { AlertRule, AlertRuleSpec, AlertRuleTestResult, AlertSeverity } from '../../services/types';
...
export function MetricsAlertsPage() {
  ...
  const contexts = useMemo(
    () => buildMetricAlertContexts(environmentsQuery.data ?? [], integrationsQuery.data ?? []),
    [environmentsQuery.data, integrationsQuery.data],
  );
  const environmentNames = useMemo(() => new Map((environmentsQuery.data ?? []).map((item) => [item.id, item.name])), [environmentsQuery.data]);
  const rules = rulesQuery.data ?? [];
  const visibleRules = useMemo(() => {
    const keyword = query.trim().toLowerCase();
    return rules.filter((rule) => {
      const environmentId = rule.spec.scope.environmentId ?? '';
      if (environmentFilter && environmentId !== environmentFilter) return false;
      if (!keyword) return true;
      return `${rule.spec.name} ${rule.spec.query.expression} ${environmentId} ${rule.spec.scope.environmentName ?? ''}`.toLowerCase().includes(keyword);
    });
  }, [environmentFilter, query, rules]);
  const loading = rulesQuery.isLoading;
  const error = rulesQuery.error;
  const contextError = environmentsQuery.error || integrationsQuery.error;
  const refresh = () => void Promise.all([rulesQuery.refetch(), environmentsQuery.refetch(), integrationsQuery.refetch()]);
  ...
  return (
    <div className="console-workbench min-h-0 overflow-auto">
      <DataPanel
        title="指标告警"
        help="规则按环境隔离，查询目标由环境当前连接的 VictoriaMetrics 写入目标决定。"
        action={(
          <div className="flex w-full flex-wrap items-center justify-end gap-2 lg:w-auto">
            <select className="console-input h-9 w-full min-w-40 lg:w-auto" value={environmentFilter} onChange={(event) => setEnvironmentFilter(event.target.value)} aria-label="筛选指标告警环境">
              <option value="">全部环境</option>
              {(environmentsQuery.data ?? []).map((environment) => <option key={environment.id} value={environment.id}>{environment.name}</option>)}
            </select>
            <div className="relative w-full lg:w-72">
              <Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted" />
              <input className="console-input h-9 w-full pl-8 text-sm" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索规则、环境或查询表达式" />
            </div>
            ...
            {contexts.length ? (
              ...
              <button type="button" className="console-button console-button-primary" disabled title="请先接入并连接一个 active 指标环境"><Plus className="h-3.5 w-3.5" />创建告警</button>
            )}
          </div>
        )}
      >
        {contextError ? <div className="console-notice console-notice-warning mb-3">环境上下文加载失败，现有规则仍可查看，创建和编辑暂不可用：{errorMessage(contextError)}</div> : null}
        {loading ? <AlertTableSkeleton /> : error ? (
          ...
          <div className="overflow-x-auto rounded-md border border-outline bg-white">
            <table className="console-table w-full min-w-[1120px] table-fixed">
              ...
              <thead><tr><th>规则</th><th>环境</th><th>查询</th><th>触发条件</th><th>通知</th><th>状态</th><th className="text-right">操作</th></tr></thead>
              <tbody>{visibleRules.map((rule) => (
                <tr key={rule.id}>
                  ...
                  <td><div className="truncate font-medium">{rule.spec.scope.environmentName || environmentNames.get(rule.spec.scope.environmentId ?? '') || '-'}</div><div className="mt-1 truncate font-mono text-[11px] text-muted">{rule.spec.scope.environmentId || '-'}</div></td>
...
function buildMetricAlertContexts(environments: PlatformEnvironment[], integrations: MetricsIntegrationView[]): MetricAlertContext[] {
  return environments
    .filter((environment) => environment.status === 'active')
    .flatMap((environment) => {
      const view = integrations.find(({ integration }) => integration.environmentId === environment.id && integration.desiredState === 'connected');
      return view ? [{ environment, integration: view.integration }] : [];
    });
}
...
function MetricsAlertEditor({ mode, ruleId, contexts, onClose }: { mode: 'create' | 'edit'; ruleId: string; contexts: MetricAlertContext[]; onClose: () => void }) {
  ...
  const selectedContext = contexts.find((context) => context.environment.id === environmentId) ?? null;
  ...
  useEffect(() => {
    if (mode !== 'create' || environmentId || !contexts[0]) return;
    setEnvironmentId(contexts[0].environment.id);
  }, [contexts, environmentId, mode]);
  ...
  useEffect(() => {
    ...
    setEnvironmentId(rule.spec.scope.environmentId ?? '');
  ...
  const spec = useMemo<AlertRuleSpec>(() => ({
    ...
    scope: {
      ...
      environmentId: selectedContext?.environment.id ?? environmentId,
      environmentName: selectedContext?.environment.name ?? '',
  ...
  }), [environmentId, evaluationInterval, expression, groupFields, name, operator, ownerTeam, policyId, queryMode, ruleQuery.data?.spec.scope.scopeLabels, selectedContext, severity, threshold, triggerWindow]);
  const inputSnapshot = JSON.stringify(spec);
  const missing = [
    !selectedContext && '已连接指标环境', !spec.name && '规则名称', !spec.query.expression && '查询表达式', !(threshold > 0) && '阈值', !policyId && '平台级通知策略', !ownerTeam.trim() && '责任团队',
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-auto bg-surface p-4">
          ...
          <EditorSection title="规则与环境">
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <Field label="指标环境"><select className="console-input mt-1 w-full" value={environmentId} disabled={mode === 'edit'} title={mode === 'edit' ? '指标告警创建后不能变更环境' : undefined} onChange={(event) => setEnvironmentId(event.target.value)}><option value="">请选择已连接环境</option>{contexts.map((context) => <option key={context.environment.id} value={context.environment.id}>{context.environment.name}</option>)}</select></Field>
            </div>
            {selectedContext ? <div className="mt-3 rounded border border-outline bg-surface-low px-3 py-2 text-xs text-muted">写入目标由环境接入关系确定：<span className="font-mono text-on-surface">{selectedContext.integration.destinationRef}</span></div> : <div className="mt-3 text-xs font-semibold text-warning">当前环境未处于 active 且指标接入 connected 状态。</div>}
          </EditorSection>
