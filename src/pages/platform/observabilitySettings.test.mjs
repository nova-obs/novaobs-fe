...
test('观测接入配置列表不展示冗余标题描述，并按领域提供精简工具栏', () => {
  ...
  assert.equal(source.includes('搜索日志数据端点、URL、集群或 Topic'), true);
  assert.equal(source.includes('搜索指标数据端点、URL 或集群'), true);
  assert.match(source, /刷新[\s\S]*新增指标数据端点[\s\S]*新增日志数据端点/);
});
