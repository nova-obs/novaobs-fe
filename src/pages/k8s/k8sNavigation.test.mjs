...
test('K8s 运维二级导航保留集群总览和观测运行时入口', () => {
  ...
  assert.equal(k8sNavigationItems.find((item) => item.id === 'observability-entry')?.label, '观测接入');
...
test('K8s 运维可按子路径定位当前二级导航', () => {
  ...
  assert.equal(getK8sNavigationByPath('/k8s/observability')?.id, 'observability-entry');
