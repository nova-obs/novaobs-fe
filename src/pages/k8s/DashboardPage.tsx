...
const resourceRows = [
  ['prod-core', 'production', 'orders', 'Deployment/orders-api', '109 / 118', 'warning'],
  ['prod-core', 'production', 'payment', 'Deployment/payment-gateway', '42 / 42', 'healthy'],
  ['prod-edge', 'edge', 'ingress', 'DaemonSet/ingress-log-agent', '18 / 18', 'healthy'],
  ['prod-data', 'production', 'storage', 'StatefulSet/victoria-logs', '7 / 8', 'warning'],
];

const eventRows = [
  ['10:31:08', 'Deployment/orders-api', 'rollout paused', 'warning'],
  ['10:28:42', 'RoleBinding/platform-reader', 'binding synced', 'healthy'],
  ['10:24:17', 'Secret/prod-kubeconfig', 'fingerprint rotated', 'healthy'],
];

export function DashboardPage() {
  const { data, error, isLoading } = useQuery({
    queryKey: ['k8s-dashboard', 'prod'],
    queryFn: () => api.getK8sDashboard('prod'),
    retry: false,
  });

  const stats = data?.stats;
  const sync = data?.sync;
  const readyPods = stats?.pods.ready ?? 109;
  const totalPods = stats?.pods.total ?? 128;
  const warningPods = stats?.pods.warning ?? 7;
    ...
    <div className="space-y-4">
      {error ? (
        <div className="console-panel flex items-center gap-3 px-4 py-3 text-sm text-warning">
          ...
          后端 K8sOps Dashboard 暂未连接，当前显示模块壳与迁移态势。
        </div>
      ) : null}
        ...
        <section className="console-panel relative min-h-[330px] overflow-hidden p-5">
          ...
          <div className="relative flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full bg-primary-soft/75 px-2 py-0.5 text-[11px] font-semibold text-primary">source: startorch</span>
              </div>
          ...
          <div className="relative mt-5 grid min-h-[240px] gap-4 lg:grid-cols-[minmax(0,1fr)_220px]">
            <div className="relative overflow-hidden rounded-lg bg-white/45 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.72)]">
              ...
              <ClusterNode className="left-[24%] top-[42%]" icon={Network} label="API Server" meta="unknown" />
              <ClusterNode className="left-[58%] top-[28%]" icon={Boxes} label="Workloads" meta={`${stats?.workloads ?? 47} active`} />
              <ClusterNode className="left-[75%] top-[62%]" icon={Database} label="Namespaces" meta={`${stats?.namespaces ?? 12} domains`} />
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <SummaryTile icon={Layers3} label="运行工作负载" value={String(stats?.workloads ?? 47)} source="Deployment / StatefulSet" />
        <SummaryTile icon={Database} label="命名空间" value={String(stats?.namespaces ?? 12)} source="tenant boundary" />
        <SummaryTile icon={ShieldCheck} label="RBAC 策略" value="36" source="Role / Binding" />
        <SummaryTile icon={KeyRound} label="凭据资产" value="8" source="Secret fingerprint" />
      </div>

      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_380px]">
        <DataPanel title="资源视图" meta="startorch capability baseline · 最近 15 分钟">
          <div className="overflow-auto">
            <table className="console-table min-w-[680px] w-full">
              ...
              <tbody>
                {resourceRows.map(([cluster, env, ns, resource, pods, state]) => (
                  <tr key={`${cluster}-${resource}`} className="bg-white/35 hover:bg-white/60">
                    <td className="font-mono text-xs">{cluster}</td>
                    <td className="text-xs text-muted">{env}</td>
                    <td className="font-mono text-xs">{ns}</td>
                    <td className="font-semibold">{resource}</td>
                    <td className="font-mono text-xs">{pods}</td>
        ...
        <DataPanel title="操作审计" meta="Audit Event · trace 预留">
          <div className="space-y-2">
            {eventRows.map(([time, target, action, state]) => (
              <div key={`${time}-${target}`} className="rounded-lg bg-white/45 px-3 py-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.68)]">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <div className="truncate text-sm font-semibold text-on-surface">{target}</div>
                    <div className="mt-1 text-[11px] text-muted">{time} · {action}</div>
                  </div>
