...
function hasActiveFilters(filters) {
  return !!(filters.q || filters.status || filters.source);
}
...
test('空 filter 对象无激活筛选', () => {
  assert.equal(hasActiveFilters({ q: '', status: '', source: '' }), false);
});

test('有 q 时判定为激活筛选', () => {
  assert.equal(hasActiveFilters({ q: 'payment', status: '', source: '' }), true);
});
